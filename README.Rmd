...
**Step 3** Install *PGDspider* and *plink*. Note these programs are required for the use of **genepop_toploc()** and **PGDspidR()** only!
...
#genepop\_toploc
This function will return Loci and associated pairwise FST values among two population groupings. The function seeks a loci panel which maximizes the summed pair-wise FST among unlinked loci. **Note warning messages and file run information are returned from both plink and PGDspider. These are known and not an issue. Fst values are successfully created for 100+ loci using hierfstat. 
