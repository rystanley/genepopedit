...
##genepop\_toploc
This function will return Loci and associated pairwise FST values among two population groupings as well as a dataframe of linked loci. The function seeks a loci panel which maximizes the summed pair-wise FST among unlinked loci. The returned object is a list containing Linked loci ($Linked), pairwise FST ($FST) and the top unlinked loci by FST ($FST_Unlinked). Note 'linked' loci are calcuated based on the threshold R<sup>2</sup> of 0.2 used by plink.
...
```r
#evaluate which loci have highest fst and are unlinked among two populations.
