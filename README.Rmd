...
#vector of sample IDs to remove
  ...
  subset_genepop_indiv(GenePop=GenePopData,indiv=subid,keep=FALSE, path=paste0(output_dir,"Genepop_IDsubset.txt"))
```
#genepop\_sample
...
#Create training dataset (keep=TRUE)
  subset_genepop_indiv(GenePopData,indiv=SubSamp,keep=TRUE,path=paste0(output_dir,"Genepop_training.txt"))

#Create an assignment dataset using the remaining individuals (keep=FALSE)
  subset_genepop_indiv(GenePopData,indiv=SubSamp,keep=FALSE,path=paste0(output_dir,"Genepop_assignment.txt"))
```
...
```r
#convert Genepop format to FSTAT *note the .dat file format
...
If you are interested in calculating summary information such as linkage disequilibrium, it is useful to convert a genepop file into a 'genotype' format which can ue used by the R package 'genetics' <https://cran.r-project.org/web/packages/genetics/index.html> 

```r
#convert Genepop format to genotype *note this returns the genotype object to the workspace.
  genepop_genotype(GenePop="Genepop_IDsubset.txt")
```
