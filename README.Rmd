...
*genepopedit* was written in collaboration:
  ...
  * Dr. Ian Bradbury <https://bradburygeneticslab.com/>
...
**r2.theshold** | Linkage threshold based on correlation between loci. Value must be between 0 & 1. Default threshold (0.2) matched to PLINK defaults. 
**FST.threshold** | The minimum r^2 threshold to consider a pair of loci to be considered linked.
...
##genepop\_detective
This function queries meta-data from your genepop data without having to open it in a text editor. Function returns a vector of population names, population counts, sample IDs, or Loci names. [_back to function descriptions_](#funcdiag)
...
<a name="genepopafreq"/>
##genepop\_allelefreq
This function will calculate allele frequencies based on the major allele among populations for each population group. [_back to function descriptions_](#funcdiag)
...
<a name="subsetgenp"/>
##subset\_genepop
Create a new Genepop file which only contains Loci *3, 15, 23, 49, 62, 81, 88, & 94*. This useful if, for example, you identify loci which are potentially under selection and want a parsed dataset. Here our loci are named in the Loci# (1:100). [_back to function descriptions_](#funcmanip)
...
Now change the names of some of the populations. In this example we will rename populations DDD & HHH to YYY & ZZZ. [_back to function descriptions_](#funcmanip)
...
_This function is best run at the beginning of the analysis prior to removal of specific loci or populations._ [_back to function descriptions_](#funcmanip)
...
<a name="genepopsamp"/>
##genepop\_sample
Function which will allow you to create subsets of the genepop. This might be useful for creating training and leave-out datasets for assignment. [_back to function descriptions_](#funcsamp)
...
<a name="genepopstruc"/>
##genepop\_structure
If you are interested in investigating population structure you can convert your modified Genepop object or path to saved file directly into a STRUCTURE formatted text (.str) file <http://pritchardlab.stanford.edu/structure.html>. [_back to function descriptions_](#funcconvert)
...
This data can be used by the R package 'hierfstat' <https://cran.r-project.org/web/packages/hierfstat/index.html> or the program 'FSTAT' <http://www2.unil.ch/popgen/softwares/fstat.htm> [_back to function descriptions_](#funcconvert)
...
<a name="genepopnh"/>
##genepop\_newhybrids
If you are interested in testing for hybridization you can convert from Genepop to the format necessary for the program New Hybrids <http://ib.berkeley.edu/labs/slatkin/eriq/software/software.htm#NewHybs>. [_back to function descriptions_](#funcconvert)
...
<a name="genepopagn"/>
##genepop\_assigner
If you are interested conducting assignment analysis, you can convert to the format required for the R package "assigner" <https://github.com/thierrygosselin/assigner>. [_back to function descriptions_](#funcconvert)
...
##genepop\_treemix
**genepopedit** is one of the few tools available to convert data to the format required for input into to the Treemix format. It creates a gzipped file that clusters your individuals based on the first 3 letters of the individual's code. Treemix can be used to infer migration weight and directionality among your populations after building a maximum likelihood phylogram <https://bitbucket.org/nygcresearch/treemix/wiki/Home>. [_back to function descriptions_](#funcconvert)

```r

# Set new population groupings (group CCC/DDD/EEE and III/JJJ)
  treegroups <- data.frame(pops = c("AAA","BBB","CCC","DDD","EEE","FFF","GGG","HHH","III","JJJ"),groups = c("1","2","3","3","3","4","5","6","7","7"))
  
#convert Genepop to Treemix input and keep intermediary conversion files (default: FALSE)
  genepop_treemix(GenePop="Genepop_IDsubset.txt",where.PLINK="c:/Users/YOURNAME/Documents/Programs/plink/",where.PGDspider="c:/Users/YOURNAME/Documents/Programs/PGDSpider_2.0.9.0/",popgroup=treegroups,keep_inter=TRUE,path=paste0(output_dir,"Treemix_IDsubset.txt"))

```

<a name="genepopflat"/>
##genepop\_flatten
Flatten your geneotype data for other calculations. This function will convert and return the genepop format into a dataframe. Because this function returns the dataframe, it needs to be assigned a variable ID. Note that this function does not have any subsetting capabilities. If you would like to remove loci and-or populations, this must be before hand using *subset_genepop* or its sister functions (see below for examples). [_back to function descriptions_](#funcconvert)
...
##genepop\_unflatten
If you are working with a flattened dataframe in your workspace you can convert it back to Genepop format. **Note that the first column of this data.frame should correspond to the sampleID (e.g. "BON_01") and the remaining columns should be Loci.[_back to function descriptions_](#funcconvert)
