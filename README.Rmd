...
#### genepop_toploci.R
Calculate the fst for unlinked loci and order by fst. This function will order loci by fst and will maximize the sum of FSTs for unlinked loci. Note this function requires both PGDspider and plink. [_See installation instructions here_](#extrainstall)
...
--------------|-----------------------------------
**Genepop** | A path to a Genepop file _or_ a dataframe read into the workspace of a Genepop file.  
**LDpop** | A string which denotes which of the populations you wish to calculate linkage disequilibrium from. The default is to calculate LD among all populations.
...
**allocate.PGD.RAM** | An integer defining how many GB of ram will be made available to PGD spider during calculation of LD. Default is 1 GB which should be sufficient for most calculations. 
...
##genepop\_toploc
This function will calculate linkage disequilibrium and global Weir and Cockerham's FST for loci (# specified by panel.size) among populations. The function seeks a loci panel which maximizes the summed global FST among unlinked loci. The returned object is a list containing Linked loci ($Linked), global FST ($FST) and the top unlinked loci by FST ($FST_Unlinked). Note 'linked' loci are calcuated based on the threshold R<sup>2</sup> of 0.2 used by plink.

**Note warning messages and file run information are returned from both plink and PGDspider. These are known and not an issue. Fst values are successfully created for 100+ loci using hierfstat. ** 

```r

#calculate fst and linkage and return output for 20 loci. To calculate LD we use PLINK which requires a file conversion using PGDspider. We will provide the function with access to 2 GB of ram (default: 1) to speed up this conversion

  TopLoci <- genepop_toploci(GenePop="genepopedit_examplefile.txt",panel.size=20,   allocate.PGD.RAM =2,
...
##genepop\_unflatten
If you are working with a flattened dataframe in your workspace you can convert it back to Genepop format. **Note that the first column of this data.frame should correspond to the sampleID (e.g. "BON_01") and the remaining columns should be Loci. 
